...
│   ├── 📄 cv.yml: CV in YAML format for the CV page
...
This site currently generates the CV page from [\_data/cv.yml](_data/cv.yml).

The upstream template also supports a JSON Resume data source, but this repository no longer ships `assets/json/resume.json` or the related loader configuration. If you want to switch back to JSON Resume later, you will need to restore both the JSON file and the corresponding settings in [\_config.yml](_config.yml).
